...
# DMFT configs:
parser.add_argument('--T0', default=10, type=float,
                    help='T0')
...
def main(args):
    ...
    args.f.write('T0={}\tcount={}\tiota={}\tmomentum={}\n'.format(args.T0, args.count, args.iota, args.momentum))
...
def main_worker(args):
    ...
    scf = DMFT(args.Tem, args.count, args.z, args.iota, args.momentum, args.device, args.double)

    # create model
    Net = args.Net[:args.Net.index('_')]
    model = Network(Net, args.input_size, args.output_size, args.embedding_size, args.hidden_size, args.z, args.hermi,
...
def train(train_loader, model, criterion, optimizer, scf, epoch, args):
    ...
    for i, (H0, E_mu, U, target) in enumerate(train_loader):
        ...
        H = H0 + torch.diag_embed(scf(H0, E_mu, U, model))  # (bz, 1, size, size)
